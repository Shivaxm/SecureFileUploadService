...
def scan_file(file_id: str) -> str:  # noqa: PLR0911, PLR0912
    ...
    try:
        ...
        if sniffed is None:
            ...
            db.commit()
            log_event(db, actor_user_id=file_obj.owner_id, action="SCAN_QUARANTINED", file_id=file_obj.id, metadata={"reason": "sniff_missing"})
            return "quarantined"

        if file_obj.size_bytes and file_obj.size_bytes > MAX_SIZE_BYTES:
            file_obj.state = models.FileObjectState.QUARANTINED
            db.commit()
            log_event(db, actor_user_id=file_obj.owner_id, action="SCAN_QUARANTINED", file_id=file_obj.id, metadata={"reason": "too_large", "size": file_obj.size_bytes})
            return "quarantined"

        declared_ok = file_obj.declared_content_type.split(";")[0] in ALLOWED_CONTENT_TYPES
        ...
        file_obj.state = models.FileObjectState.QUARANTINED
        db.commit()
        log_event(db, actor_user_id=file_obj.owner_id, action="SCAN_QUARANTINED", file_id=file_obj.id, metadata={"reason": "disallowed_type", "sniffed": sniffed, "declared": file_obj.declared_content_type})
        return "quarantined"

    except Exception as exc:  # noqa: BLE001
        log_event(db, actor_user_id=None, action="SCAN_FAIL", file_id=file_id, metadata={"error": str(exc)})
        raise
    finally:
        db.close()
